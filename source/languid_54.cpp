...
NumericMatrix raster_data(int iter, int layers, int pixels, double zoom, double alpha) {
  ...
  for(int it = 1; it < iter; it++) {
    ...
    if(variant == 0) {
      x = (heart_y(x) * 1.85 / 2 - .125) + (heart_x(x) * 1.85 / 2);
      y = (heart_y(y) * 1.85 / 2 - .125) + (heart_x(y) * 1.85 / 2);
      z = x + y;
    } else if (variant == 1) {
      // s = sin(x + y) * 3.14 * l;
      // f = (1 - fabs(sin(x + y))) * .1 * it/iter + .9;
      // z = z/2;
      // x = heart_x(s + rot1) * r * f / 1.2 + .5;
      // y = heart_y(s + rot1) * r * f / 1.2 + .5;
    } else if (variant == 2){
      x = sin(x) * 2;
      y = -sin(y) * 2;
      z = sin(z) * 2;
      ...
      // x = heart_x(s + rot2) * r * (iter - it)/iter / 2 + .5;
      // y = heart_y(s + rot2) * r * (iter - it)/iter / 2 + .5;
      // z = s;
    }
