...
NumericMatrix raster_data(int iter, int layers, int pixels, double zoom, double alpha) {
  ...
  for(int it = 1; it < iter; it++) {
    ...
    if(variant == 0) {
      ...
      s = sin(x + y) * 3.14 / 2;
      z = sin(x + y);
      x = heart_x(s + rot) * r * (iter - it)/iter;
      y = heart_y(s + rot) * r * (iter - it)/iter;
    } else if (variant == 2){
      x = sin(x) * 2;
      y = sin(y) * 2;
      z = sin(z) * 2;
      ...
      s = sin(x + y) * 3.14 / 2;
      x = heart_x(s + rot) * r * (iter - it)/iter / 2;
      y = heart_y(s + rot) * r * (iter - it)/iter / 2;
      z = sin(z);
    }
